...
CREATE TABLE DV (
    ...
    gia DECIMAL(18,2),
    loaiDV NVARCHAR(20) CHECK (loaiDV IN (N'THUCAN', N'MAYGIAT', N'NUOCUONG')),
    mieuTa NVARCHAR(255),
    donVi NVARCHAR(20)
);
...
CREATE TABLE DichVuSuDung (
    ...
    giaDV DECIMAL(18,2),
    trangThai BIT,
    CONSTRAINT PK_DVSD PRIMARY KEY (maDV, maHD),
    CONSTRAINT FK_DVSD_DV FOREIGN KEY (maDV) REFERENCES DV(maDV),
    CONSTRAINT FK_DVSD_HD FOREIGN KEY (maHD) REFERENCES HoaDon(maHD)
);
