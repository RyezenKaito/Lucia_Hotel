...
public class NhanVienView extends BorderPane {
    ...
    private VBox buildTable() {
        ...
        table.setRowFactory(tv -> {
            ...
            row.setOnMouseEntered(e -> {
                if (!row.isEmpty()) row.setStyle("-fx-background-color: #f0f4ff;");
            });
        ...
        TableColumn<NhanVien, String> colHeSo = new TableColumn<>("Hệ số lương");
        colHeSo.setCellValueFactory(c -> new SimpleStringProperty(
            String.valueOf(c.getValue().getHeSoLuong())));
        colHeSo.setMinWidth(100);
        colHeSo.setStyle("-fx-alignment: CENTER;");

        TableColumn<NhanVien, String> colChucVu = new TableColumn<>("Chức vụ");
        colChucVu.setCellValueFactory(c -> new SimpleStringProperty(
            c.getValue().getRole() == ChucVu.QUAN_LY ? "Quản lý" : "Nhân viên"));
        ...
        table.getColumns().addAll(colSTT, colMa, colTen, colSDT, colNgayVao, colHeSo, colChucVu);
        ...
        table.setRowFactory(tv -> {
            ...
            row.setOnContextMenuRequested(e -> {
                ...
                boolean isTargetManager = (nv.getRole() == ChucVu.QUAN_LY);

                // Manager chỉ sửa/xóa staff, không sửa/xóa manager cùng/trên cấp
                miEdit.setDisable(isTargetManager);
                miDelete.setDisable(isTargetManager);
            });
    ...
    public void loadData() {
        ...
        for (NhanVien nv : all) {
            // Manager không thấy chính mình trong danh sách
            if (currentUser != null && nv.getMaNV().equals(currentUser.getMaNV())) continue;

            masterData.add(nv);

            if (nv.getRole() == ChucVu.QUAN_LY) totalManager++;
            else totalStaff++;
        }
    ...
    private void applyFilter(String keyword) {
        ...
        if (keyword == null || keyword.isBlank()) {
            ...
            filteredData.setPredicate(nv ->
                nv.getMaNV().toLowerCase().contains(lower) ||
    ...
    private void confirmDelete(NhanVien nv) {
        ...
        alert.setContentText("Bạn có chắc muốn xóa nhân viên: " + nv.getHoTen() + " ?");
