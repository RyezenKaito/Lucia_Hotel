...
public final class ValidationUtils {
    ...
    /** * HỌ VÀ TÊN: 
     * - Chỉ cho phép chữ cái (hỗ trợ 100% tiếng Việt), khoảng trắng, nháy đơn, gạch nối.
     */
     ...
     * - Máy bàn (11 số): 02 + 9 số.
     */
    public static final String REGEX_PHONE_VN = "^(02\\d{9}|(03[2-9]|05[25689]|07[06-9]|08[1-9]|09[0-46-9])\\d{7})$";
    ...
    public static boolean isValidCCCDCenturyAndGender(String cccd, int namSinh) {
        if (cccd == null || cccd.length() < 4) return false;
        
        int theKyGioiTinh = cccd.charAt(3) - '0';
        
        // Quy định mã thế kỷ & giới tính của Bộ Công An (Nam chẵn, Nữ lẻ)
        if (namSinh >= 1900 && namSinh <= 1999) return (theKyGioiTinh == 0 || theKyGioiTinh == 1);
        if (namSinh >= 2000 && namSinh <= 2099) return (theKyGioiTinh == 2 || theKyGioiTinh == 3);
        if (namSinh >= 2100 && namSinh <= 2199) return (theKyGioiTinh == 4 || theKyGioiTinh == 5);
        if (namSinh >= 2200 && namSinh <= 2299) return (theKyGioiTinh == 6 || theKyGioiTinh == 7);
        if (namSinh >= 2300 && namSinh <= 2399) return (theKyGioiTinh == 8 || theKyGioiTinh == 9);
    ...
    public static boolean isValidCCCDBirthYear(String cccd, int namSinh) {
        if (cccd == null || cccd.length() < 6) return false;
 ...
 ══════════════════════════════════════════════════════════════════ */
     ...
     public static void applyNumericOnlyFilter(javafx.scene.control.TextField textField, int maxLength) {
         ...
         // Tạo bộ lọc: Chặn nếu ký tự mới thêm vào KHÔNG phải là số, hoặc tổng độ dài vượt quá maxLength
         javafx.scene.control.TextFormatter<String> formatter = new javafx.scene.control.TextFormatter<>(change -> {
             ...
             // Nếu chuỗi mới rỗng (người dùng đang xóa) HOẶC chuỗi mới toàn là số và không vượt quá giới hạn
